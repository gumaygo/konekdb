...
module.exports = HandleMsg = async (pakforlay, message) => {
	...
	/*	const isSimi = simi.includes(chatId) */
        ...
        const isRegistered = await connection.cekUser(pengirim.split('@')[0]);
	...
	// Filter Banned People
        ...
        case 'adduser':
            ...
            if (args.length == 0) return pakforlay.reply(from, `Jika kalian ingin menghapus user silahkan\nketik ${prefix}6285325xxxx`, id)
            let cek1 = await connection.cekUser(args[0].replace('@', ''))
            if(cek1 == true) {
                await pakforlay.reply(from, `User Sudah Ada`, id)
            } else {
                await connection.tambahUser(args[0].replace('@', ''))
                await pakforlay.reply(from, `Sukses Add ${args[0].replace('@', '')}`)
            }
            break
        ...
        case 'deleteuser':
            if (!isOwnerBot) return pakforlay.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return pakforlay.reply(from, `Jika kalian ingin menghapus user silahkan\nketik ${prefix}6285325xxxx`, id)
            let cek2 = await connection.cekUser(args[0].replace('@', ''));
            if(cek2 != true) {
                await pakforlay.reply(from, `User Tidak Ada`, id)
            } else {
                await connection.hapusUser(args[0].replace('@', ''))
                await pakforlay.reply(from, `Sukses Delete ${args[0].replace('@', '')}`)
            }
            break
        // Menu and TnC
        case 'speed':
        case 'ping':
            if (!isRegistered) return pakforlay.reply(from, 'Anda Belum Terdaftar!', id)
